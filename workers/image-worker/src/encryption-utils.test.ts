...
describe('image-worker encryption-utils', () => {
  ...
  it('encrypts and decrypts binary payloads', async () => {
    ...
    const payload = new TextEncoder().encode('striae-image-binary');

    const encrypted = await encryptBinaryForStorage(payload.buffer, publicKeyPem, 'img-key-v1');

    expect(encrypted.envelope.algorithm).toBe('RSA-OAEP-AES-256-GCM');
    expect(encrypted.envelope.encryptionVersion).toBe('1.0');
    expect(encrypted.envelope.keyId).toBe('img-key-v1');
    expect(encrypted.ciphertext.byteLength).toBeGreaterThan(0);

    const decrypted = await decryptBinaryFromStorage(
